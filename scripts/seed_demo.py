...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        "--areas", nargs="+", default=["NO2"], help="price areas to seed (MVP focuses on NO2 only)"
    )
    ap.add_argument("--windsim-portfolio", default="north", help="windsim source portfolio")
    ap.add_argument("--keep", action="store_true", help="do not wipe existing portfolios first")
...
def main() -> None:
    ...
    for area in args.areas:
        ...
        subprocess.run(
            [
                sys.executable,
                str(PROJECT_ROOT / "scripts" / "load_windsim_data.py"),
                "--area",
                area,
                "--user",
                f"Wind Co {area}",
                "--windsim-portfolio",
                args.windsim_portfolio,
            ],
            check=True,
        )
    ...
    with get_session() as s:
        ...
        print("=" * 56)
    print(
        "Tip: realised prices exist only where load_actuals.py has run (NO2 here); "
