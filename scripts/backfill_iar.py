...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        "--capacity-mw", type=float, default=100.0, help="installed capacity (sigma basis)"
    )
...
def main() -> None:
    ...
    with get_session() as s:
        ...
        print(bar)
        print(
            f"IaR backfill | area={args.area} portfolio='{pf_name}' (#{portfolio_id}) "
        ...
        for run in runs:
            ...
            print(
                f"  {run.results[0].horizon} | vintage {run.vintage_ts}  "
